...
for several regression (linear, asymmetric linear, median and quantile regression)
and mixture models.
...
\item BMS in linear regression with non-normal residuals \citep{rossell:2016b}. Particular cases are Bayesian versions
...
\cite{rossell:2016} developed a general posterior sampling framework for NLPs based on Gibbs sampling
