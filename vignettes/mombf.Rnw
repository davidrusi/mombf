...
for regression (linear, asymmetric linear, median and quantile regression)
and mixture models.
This is the main package implementing {\it non-local prior} (NLP) distributions
(briefly reviewed here, see \cite{johnson:2010,johnson:2012} for a more detailed treatment),
although other priors are also implemented (e.g. Zellner's prior in regression, Normal-IWishart priors for mixtures).
...
(Section \ref{sec:bma}, \cite{rossell:2017}).
...
\item BMS in linear regression with non-normal residuals \citep{rossell:2017b}. Particular cases are Bayesian versions
  of asymmetric least squares, median and quantile regression.

\item BMS for mixture models (Section \ref{sec:mixtures}, currently only Normal mixtures), see \cite{fuquene:2018}.
\end{itemize}
...
We now briefly discuss how to set the prior dispersion $\tau$ and propose default values,
though we encourage everyone to think which values are more appropriate for the current problem at hand.
By default we set $\tau$ so that $P(|\theta/\sqrt{\phi}|>0.2)=0.99$,
...
The function \texttt{priorp2g} facilitates finding $\tau$ for any given threshold.
...
Prior densities can be evaluated and plotted as shown below.
