...
for regression (linear, asymmetric linear, median and quantile regression) and mixture models.
...
The main function of interest for many users is \texttt{modelSelection} performing BMS and BMA (the latter for some selected models, mainly linear regression and Normal mixture models).
