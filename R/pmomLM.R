...
for (i in 2:niter) {
  ...
  for (j in 1:p1) {
    ...
    newval <- MHTheta1pmom(ej,j=j,delta=curDelta,theta1=curTheta1,phi=postPhi[i-1],r=r,tau=postTau[i-1],xj=x[,j],modelPrior=modelPrior)
...
MHTheta1pmom <- function(e,j,delta,theta1,phi,r,tau,xj,modelPrior) {
