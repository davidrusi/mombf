...
We implement both product and quadratic non-local priors.
Quadratic non-local priors are historically the first form of non-local priors that were introduced,
...
\subsection{Quadratic non-local priors}
\label{ssec:quadpriors}
...
We define the quadratic MOM prior as
\begin{equation}
...
The quadratic iMom prior on $\bm{\theta}_1$ is
...
\texttt{penalty=='quadratic'} for the quadratic priors.
...
for $\theta_1 \in (-3,3)$.
Notice that in the univariate case the product and quadratic priors are equivalent.
...
The examples use quadratic non-local priors.
...
save considerable computing time, as the Monte Carlo samples need only be generated
...
In the examples we use quadratic non-local priors.
For examples using product non-local priors see Section \ref{sec:varsellm}.
...
priorDelta <- new("msPriorSpec",priorType='modelIndicator',priorDistr='binomial',priorPars=c(alpha.p=1,beta.p=1))
